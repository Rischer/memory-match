<template>
	...
	>
		...
		<div class="text-center mb-8">
			...
			<div class="flex justify-center gap-4 mb-4">
				<div class="bg-white p-3 rounded-lg shadow">
					<span class="font-semibold text-purple-600">{{ t('stats.moves') }}:</span>
					<span class="ml-1">{{ moves }}</span>
				</div>
				<div class="bg-white p-3 rounded-lg shadow">
					<span class="font-semibold text-purple-600">{{ t('stats.pairs') }}:</span>
					<span>{{ matchedCards.length / 2 }}</span>/<span>{{ currentDifficulty.pairs }}</span>
				</div>
			</div>
			<button
				@click="resetGame"
		...
		<div 
			...
			:class="[
				currentDifficulty.name === 'Easy' ? 'grid-cols-4 sm:max-w-2xl' : 
				currentDifficulty.name === 'Medium' ? 'grid-cols-5 sm:max-w-3xl' : 
			...
			>
				<div class="card-inner">
					<div class="card-front">
						<Icon v-if="card.isMatched" :icon="card.icon" :class="cardSizeClass" />
					</div>
					<div class="card-back">
